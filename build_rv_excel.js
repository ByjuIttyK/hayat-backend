...
/**
 * build_rv_excel.js
 * Pure Node.js / ExcelJS replacement for build_rv_template.py
...
async function buildWorkbook(header, invoices) {
  ...
  const ws = wb.addWorksheet("RV Entry", {
  ...
  r1.value     = "RECEIPT VOUCHER (RV) — EXCEL ENTRY";
  r1.fill      = solid(C.NAV);
  ...
  lbl(ws.getCell(5,1), "RV Date");
  locked(ws.getCell(5,2), h.rvDate||"");
  ...
  // Row 6: Customer | Bank
  lbl(ws.getCell(6,1), "Cust (Cr.)");
  locked(ws.getCell(6,2), h.custCode||"", {bold:true});
  ws.mergeCells(6,3,6,5);
  locked(ws.getCell(6,3), h.custName||"", {italic:true});
  lbl(ws.getCell(6,6), "Bank (Dr.)");
  ...
  secHdr(ws, 9,
    "  SECTION D — INVOICE SETTLEMENTS  →  adj_dtl  (Src_Type='03')  |  Fill col I (Settle Amt) only — last column",
    C.SEC_D);
  ...
  for (let idx=0; idx<20; idx++) {
    ...
    locked(ws.getCell(row,7), h.custCode||"");
    // Col 8: Src Type (always '03', locked)
    const st = ws.getCell(row,8);
    st.value="03"; st.fill=solid(C.AUTO_BG);
  ...
  dn.value = `ℹ  ${iv.length} outstanding invoice(s) pre-loaded. Fill ONLY column I (Settle Amt) — last column. Leave 0 for invoices not being settled.`;
  dn.fill  = solid(C.YLW_BG);
  dn.font  = font({sz:7, italic:true, color:"FF555555"});
  dn.alignment = align("left");

  // ── SECTION B ────────────────────────────────────────────────────────────
  // New column order: # | Tran | Ref No | Vchr No | Deposit Bank | Bank Name
  //                   | Chq No * | Chq Date * | Drawn Bank | Amount *  (yellow at END)
  const BS=DN+3, BSTART=BS+2, BEND=BSTART+5, BT=BEND+1;
  secHdr(ws, BS,
    "  SECTION B — CHEQUES RECEIVED  →  pdc_rcd  (Tran_Type='03')  |  Fill Chq No, Chq Date, Drawn Bank, Amount (last 4 columns)",
    C.SEC_B);

  ws.getRow(BS+1).height = 28;
  const bHdrs = ["#","Tran","Ref No","Vchr No","Deposit Bank\n(pre-filled)","Bank Name","Chq No *","Chq Date *","Drawn Bank","Amount *"];
  ...
  for (let idx=0;idx<6;idx++) {
    ...
    t2.value="03"; t2.fill=solid(C.AUTO_BG);
    ...
    // Col 5: Deposit Bank Code (pre-filled, locked)
  ...
  const instrRows = [
    { text:`RV EXCEL — REF: ${h.refNo||""}`, bold:true, sz:12, bg:C.NAV,   fg:"FFFFFFFF" },
...
// ── Express route ─────────────────────────────────────────────────────────────
router.post("/build-rv-excel", async (req, res) => {
